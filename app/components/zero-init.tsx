import {Zero, ZeroOptions, syncedQuery} from '@rocicorp/zero';
import {ZeroProvider} from '@rocicorp/zero/react';
import {builder, schema, Schema} from 'zero/schema';
  ...
  const opts = useMemo(() => {
    return {
      ...
      init: (zero: Zero<Schema, Mutators>) => {
        ...
        setTimeout(() => {
          zero.preload(preload(), {
            ttl: '1m',
          });
        }, 1_000);
      },
    } as ZeroOptions<Schema, Mutators>;
  }, [session.data?.userID, router]);

  return <ZeroProvider {...opts}>{children}</ZeroProvider>;
}

export const preload = syncedQuery('preload', z.tuple([]), () => {
  ...
  return builder.artist.orderBy('popularity', 'desc').limit(1_000);
});
